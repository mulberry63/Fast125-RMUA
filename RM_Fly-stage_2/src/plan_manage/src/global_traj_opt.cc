...
bool GlobTrajOpt::generate_traj(const Eigen::MatrixXd &iniState,
  ...
  // std::cout << "[before] T: " << Ts.transpose() << std::endl;
  ...
  forwardT(t, T);
  // std::cout << "[after] P: \n" << P << std::endl;
  // std::cout << "[after] T: " << T.transpose() << std::endl;
...
double GlobTrajOpt::calTimeIntPenalty() {
  ...
  for (int i = 0; i < N_; ++i) {
    ...
    jerkOpt_.gdC.block<6, 3>(i * 6, 0) +=
        step_vaj * (integral_grad_Jv_to_c * config_.pnlV +
                    ...
                    +integral_grad_Jminthr_to_c * config_.pnlThr +
    ...
    jerkOpt_.gdT(i) += integral_grad_Jv_to_T * config_.pnlV +
                       ...
                       +integral_grad_Jminthr_to_T * config_.pnlThr +
  ...
  // std::cout << "thrmin_pnt_: "<<thrmin_pnt_<<" thrmax_pnt_: "<<thrmax_pnt_<<std::endl;
...
double GlobTrajOpt::objectiveFunc(void *ptrObj, const Eigen::VectorXd& x, Eigen::VectorXd &grad) {
  ...
  tailstate.col(1) = endvel; tailstate.col(2) = endacc;
  ...
  // std::cout <<"-------------------------------------------------------------------\n";
  // std::cout << "t1-t0 "<< double(t1-t0)/CLOCKS_PER_SEC * 1000.0 << "ms" <<std::endl;
  // std::cout << "t2-t1 "<< double(t2-t1)/CLOCKS_PER_SEC * 1000.0 << "ms" <<std::endl;
  // std::cout << "t3-t2 "<< double(t3-t2)/CLOCKS_PER_SEC * 1000.0 << "ms" <<std::endl;
  // std::cout << "t4-t3 "<< double(t4-t3)/CLOCKS_PER_SEC * 1000.0 << "ms" <<std::endl;
  // std::cout << "t5-t4 "<< double(t5-t4)/CLOCKS_PER_SEC * 1000.0 << "ms" <<std::endl;
...
bool GlobTrajOpt::generate_racing2_traj(const Eigen::MatrixXd &iniState,
                                const Eigen::MatrixXd &finState,
                                const Eigen::MatrixXd &waypoints,
                                const Eigen::VectorXd &Ts, Trajectory &traj) {
  ini_state_ = iniState;
  end_state_ = finState;
  end_state_.col(0)[0] += 0.31;
  end_state_.col(0)[1] += 0.0;
  end_state_.col(0)[2] += 0.17;
  iter_num = 0;
  double Ts_sum(Ts.sum());
  printf(CHALK_YELLOW("original Ts: %lf\n"), Ts_sum);
  ...
  roundingState(end_state_);
  setBoundConds(ini_state_, end_state_);
  //we optimize the end velocity and Thrust
  x_.resize(dim_t_ + 4);  
  Eigen::Map<Eigen::VectorXd> t(x_.data(), dim_t_);
  t = t_;
  ...
  memcpy(x_.data()+dim_t_,end_state_.col(1).data(), 3 * sizeof(x_[0])); //end vel
  memcpy(x_.data()+dim_t_+3,&fthr, 1 * sizeof(x_[0])); //end Thrust
  ...
  jerkOpt_.generate(waypoints, T);
  traj = jerkOpt_.getTraj();
  //add  landing traj  transform traj -> hover
  Eigen::Vector3d endpos, endvel, endacc; double duration;
  duration = 2.0;
  endpos = traj.getPos(traj.getTotalDuration());
  endvel = traj.getVel(traj.getTotalDuration());
  endacc = traj.getAcc(traj.getTotalDuration());
  BoundaryCond boundary;
  boundary << endpos,
              endvel,
              endacc,
              endpos + Eigen::Vector3d(endvel[0]*duration / 1.5, endvel[1]*duration / 1.5, -1.6),
              Eigen::Vector3d(0.0,0.0,0.0),
              Eigen::Vector3d(0.0,0.0,0.0);
      
  Piece landing_traj(boundary,duration);
  std::cout << "landing_traj: "<<landing_traj.getDuration() << "p: "<< landing_traj.getPos(landing_traj.getDuration()).transpose()<<std::endl;
  traj.append(landing_traj);
  return true;
}

int GlobTrajOpt::optimize_r2(const double &delta) {
  // Setup for L-BFGS solver
  lbfgs::lbfgs_parameter_t lbfgs_params;
  lbfgs_params.mem_size = 256;
  lbfgs_params.past = 10;
  ...
  std::clock_t start = std::clock();
    auto ret = lbfgs::lbfgs_optimize(
      x_, minObjective, GlobTrajOpt::objectiveFunc_R2, nullptr,
      nullptr, this, lbfgs_params);
  std::clock_t end = std::clock();
...
double GlobTrajOpt::objectiveFunc_R2(void *ptrObj, const Eigen::VectorXd& x, Eigen::VectorXd &grad) {
  std::clock_t t0 = std::clock();
  ...
  //hzc
  Eigen::Map<const Eigen::VectorXd> endvel(x.data()+obj.dim_t_, 3);
  double endthr =  x[x.size()-1];
  Eigen::Map<Eigen::VectorXd> gradendv(grad.data()+obj.dim_t_,3);

  Eigen::Matrix3d tailstate = obj.jerkOpt_.tailPVA;
  tailstate.col(1) = endvel; 
  tailstate.col(2) <<-endthr * sin(obj.des_EndROll),0,endthr * cos(obj.des_EndROll)-9.81;
  ...
  Eigen::Vector3d gradenda = obj.jerkOpt_.gdTail.col(2);
  // gradendv.setZero();
  // gradenda.setZero();

  grad[grad.size()-1]= gradenda.dot(Eigen::Vector3d(-sin(obj.des_EndROll),0,cos(obj.des_EndROll)));
