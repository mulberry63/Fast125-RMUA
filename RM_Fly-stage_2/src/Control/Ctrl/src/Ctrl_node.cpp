...
int main(int argc, char* argv[]) {
    ...
    rpg_mpc::MpcParams<float> mpc_param;
    mpc_param.config_from_ros_handle(nh);
    rpg_mpc::MpcController<float> mpccontroller(mpc_param);
    CtrlFSM fsm(param, controller, hov_thr_kf,mpccontroller);
    pFSM = &fsm;
    ...
    ros::Subscriber mpc_traj_sub =
        nh.subscribe<quadrotor_msgs::Trajectory>("traj_cmd",
                                                      100,
