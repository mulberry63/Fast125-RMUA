...
public:
    ...
    void convert_world_prior_info_2_local(void)
    {
        ...
        std::cout << "[debug loop Transition]   :   " << loop_odom_T.transpose() << std::endl;
    ...
    {   
        ...
        if (have_odom_msg)
        {
            ...
            std::cout << "[debug depth min_time_gap]    :   " << min_time_gap << std::endl;
            ...
            if (T_have_result)
            {
                ...
                // for (size_t k = 0 ; k < T_result_cluster.size() ; k++)
                // {
                //     Eigen::Vector3d tmp = T_result_cluster[k];
                //     pcl::PointXYZ pt;
                //     pt.x = tmp(0);
                //     pt.y = tmp(1);
                //     pt.z = tmp(2);
                ...
                // }
                // Eigen::Vector3d digit_pos;
                // if (Determine_the_position_of_the_transition_by_clustered_pointcloud(input_pc,digit_pos))
                //     T_result_odom(1) = digit_pos(1);
                ...
                if (T_pos_stamp_pair_vec.size() > config.transition_vec_size)
                {
                    ...
                    double vel = 0.0;
                    bool IsTruth = dealWithTransition(T_pos_stamp_pair_vec,vel); 

                    geometry_msgs::PoseStamped debug_msg;
                    debug_msg.pose.position.x = T_result_odom(0);
                    debug_msg.pose.position.y = T_result_odom(1);
                    debug_msg.pose.position.z = T_result_odom(2);
                    debug_msg.pose.orientation.w = std::abs(vel);
        ...
        if (have_rgb_msg && !have_alien_flag_depth)
        {
            if (std::abs(current_odom(0)-alien_gate(0)) < 10 && std::abs(current_odom(1) - alien_gate(1)) < 40)
            {
                ...
                std::cout << "[debug rgb min_time_gap]    :   " << min_time_gap << std::endl;
                        ...
                        if (!clusters.empty())
                        {
                            ...
                            {   
                                ...
                                {	
                                    ...
                                    // std::cout << "[debug ellipse result] :   " << alien_result_pos.transpose() << std::endl;
                                ...
                                // std::cout << "[debug beam result]   :   " << alien_result_pos_beam.transpose() << std::endl;
                ...
                if( (alien_result_pos - alien_gate).norm() < config.a_result_tolerance)
                {
                    ...
                    std::cout << "[debug rgb alien circle position] :   " << alien_result_pos.transpose() << std::endl;
                    std::cout << "[debug current odom]  :   " << current_odom.transpose() << std::endl;
                    std::cout << "[debug distance 2 alien circle]   :   " << (current_odom - alien_result_pos).norm() << std::endl;
    ...
    void update_circle_prior_position_by_realtive_position(const Eigen::Vector3d &detected_circle_center)
    {
        ...
        {   
            ...
            std::cout << "[debug drift] :   " << tmp.transpose() << std::endl;
        ...
        if ((detected_circle_center - rotation_gate).norm() < 3 && (current_odom - corresponding_circle).norm() > config.update_min_dis)
        {
            ...
            std::cout << "[debug before update alien] " << alien_gate.transpose() << std::endl;
            rotation_gate = detected_circle_center;
            alien_gate = rotation_gate + alien_circle_relative_2_rotation_circle_;
            std::cout << "[debug after update alien] " << alien_gate.transpose() << std::endl;
            std::cout << "[debug [alien] circle updated!!!]" << std::endl;
        }
    }
    ...
    */
                                                                    ...
                                                                    Eigen::Vector3d &e1)
    {
        ...
        {
            std::cout << "[debug]   clusters[" << i << "] : " << clusters[i].indices.size() << std::endl;
        }
        ...
        std::cout << "[debug circle_center] : " << circle_center.transpose() << std::endl;
        ...
        std::cout << "[debug digit_center] : " << digit_center.transpose() << std::endl;

        //cal rotation center
        Eigen::Vector3d direc = digit_center - circle_center;
        direc.normalize();
        center = (circle_center + direc * 2);
        std::cout << "[debug estimated rotation origin]  : " << center.transpose() << std::endl;
        std::cout << "[debug e1]" << e1.transpose() << std::endl;
                            ...
                            const double min_cluster_size,const double max_cluster_size)
    {
        ...
        */
        for (std::vector<pcl::PointIndices>::const_iterator i = clusters.begin(); i != clusters.end(); ++i)
        {
            ...
            std::cout << "[debug evalue] : " << evalue.transpose() << std::endl;
            std::cout << "[debug ratio1 & ratio2] : " << ratio1 << " , " << ratio2 << std::endl;
        ...
        {
            std::cout << "[debug] gate_list[" << i << "] = " << gate_list[i].transpose() << std::endl;
        }

        for (int i = 0; i < cluster_origin_vec.size(); i++)
        {
            std::cout << "[debug] cluster_origin_vec[" << i << "] = " << cluster_origin_vec[i].transpose() << std::endl;
        }

        for (int i = 0; i < candicates.size(); i++)
        {
            std::cout << "[debug] candicates[" << i << "] = " << candicates[i] << std::endl;
        }
        std::cout << "getCircleclusterPos" << std::endl;
        ...
        for (size_t k = 0;k < candicates.size();k++)
        {
            ...
            for (size_t m = 0; m < gate_list.size(); m++)
            {
                ...
                std::cout << "[debug] cluster_origin_vec[" << final_index << "] to gate[" << m << "] = " << tmp_dis << std::endl;
    ...
    bool dealWithTransition(std::vector<std::pair<Eigen::Vector3d,ros::Time>> &T_pos_stamp_pair_vec,double &vel)
    {
        static bool first_flag = true;
        static int last_vel_direction = 0;
        static std::vector<double> vel_vec;
        static double absolute_mean_vel;
        double  curr_pos_y = T_pos_stamp_pair_vec.back().first(1);
        vel =  (T_pos_stamp_pair_vec.back().first(1) - T_pos_stamp_pair_vec.begin()->first(1) ) /
                     ( T_pos_stamp_pair_vec.back().second - T_pos_stamp_pair_vec.begin()->second).toSec();

        Eigen::Vector3d current_pos = T_pos_stamp_pair_vec.back().first;
        
        int current_vel_direction = vel > 0 ? 1 : -1;
        ...
        if ( std::abs(vel) > 0.6 * absolute_mean_vel )
        {
            vel_vec.push_back(vel);
            absolute_mean_vel = absolute_mean_vel + 1.0 / vel_vec.size() * (std::abs(vel) - absolute_mean_vel);
        }

        // std::cout << "[debug    t_k+1]  :   " << T_pos_stamp_pair_vec.back().second.toSec() << std::endl;
        // std::cout << "[debug    t_k]  :   " << T_pos_stamp_pair_vec.begin()->second.toSec() << std::endl;
        // std::cout << "[debug    diff_t]  :   " << ( T_pos_stamp_pair_vec.back().second - T_pos_stamp_pair_vec.begin()->second).toSec() << std::endl;
        // std::cout << "[debug    p_k+1]  :   " << T_pos_stamp_pair_vec.back().first(1) << std::endl;
        // std::cout << "[debug    p_k]  :   " << T_pos_stamp_pair_vec.begin()->first(1) << std::endl;
        // std::cout << "[debug    diff_p]  :   " << (T_pos_stamp_pair_vec.back().first(1) - T_pos_stamp_pair_vec.begin()->first(1) ) << std::endl;
        // // std::cout << "[debug    mean_vel]   :   " << mean_vel << std::endl;
        // std::cout << "[debug    vel]   :   " << vel << std::endl;
        // // std::cout << "[debug    vel_vec size]   :   " << vel_vec.size() << std::endl;
        vel = current_vel_direction * absolute_mean_vel;

        //vel reverse occur, then we believe vel will not reverse in the next short time
        if (!first_flag && (last_vel_direction * current_vel_direction) == -1)
        {
    ...
    bool Determine_circle_center_by_pointcloud(const std::vector<Eigen::Vector3d> &input_cloud,Eigen::Vector3d &circle_center)
    {
        ...
        std::cout << "[debug evector] : " << std::endl << evector << std::endl;
