...
namespace my_planner
{
    ...
    void minsnapCloseform::Init(const vector<Eigen::Vector3d> &waypoints, double meanvel)
    {
        ...
        end_vaj = Eigen::MatrixXd::Zero(3, 3);
        mean_vel = meanvel;
        ts_init = false;
    ...
    void minsnapCloseform::set_end_state(const Eigen::MatrixXd &vaj)
    {
        end_vaj = vaj;
    }
    ...
    std::pair<Eigen::VectorXd, Eigen::VectorXd> minsnapCloseform::MinSnapCloseFormServer(const Eigen::VectorXd &wp, const Eigen::Vector3d &start_vaj, const Eigen::Vector3d &end_vaj)
    {
        ...
        start_cond << wp.head(1), start_vaj;
        end_cond << wp.tail(1), end_vaj;
    ...
    void minsnapCloseform::calMinsnap_polycoef()
    {
        ...
        Eigen::Vector3d start_vaj_x, start_vaj_y, start_vaj_z;
        Eigen::Vector3d end_vaj_x, end_vaj_y, end_vaj_z;
        for (int i = 0; i < n_seg + 1; i++)
        {
            wps_x(i) = wps[i](0);
            wps_y(i) = wps[i](1);
        ...
        start_vaj_x = sta_vaj.col(0);
        start_vaj_y = sta_vaj.col(1);
        start_vaj_z = sta_vaj.col(2);

        end_vaj_x = end_vaj.col(0);
        end_vaj_y = end_vaj.col(1);
        end_vaj_z = end_vaj.col(2);

        calQ();
        calM();
        std::pair<Eigen::VectorXd, Eigen::VectorXd> return_vel;
        return_vel = MinSnapCloseFormServer(wps_x, start_vaj_x, end_vaj_x);
        poly_coef_x = return_vel.first;
        dec_vel_x = calDecVel(return_vel.second);
        return_vel = MinSnapCloseFormServer(wps_y, start_vaj_y, end_vaj_y);
        poly_coef_y = return_vel.first;
        dec_vel_y = calDecVel(return_vel.second);
        return_vel = MinSnapCloseFormServer(wps_z, start_vaj_z, end_vaj_z);
