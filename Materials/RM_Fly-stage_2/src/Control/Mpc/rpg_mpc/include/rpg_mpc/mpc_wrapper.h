...
class MpcWrapper
{
 public:
  ...
  bool setCosts(
    const Eigen::Ref<const Eigen::Matrix<T, kCostSize, kCostSize>> Q,
    const Eigen::Ref<const Eigen::Matrix<T, kInputSize, kInputSize>> R,
    T state_cost_scaling = 0.0, T input_cost_scaling = 0.0);
  ...
  bool prepare();
  // void setDynamicParams(const double mass);
 ...
 private:
  ...
  Eigen::Matrix<T, kRefSize, kRefSize> W_ = (Eigen::Matrix<T, kRefSize, 1>() <<
    0.5 * Eigen::Matrix<T, 3, 1>::Ones(),
    0.1 * Eigen::Matrix<T, 4, 1>::Ones(),
    0.05 * Eigen::Matrix<T, 3, 1>::Ones(),
