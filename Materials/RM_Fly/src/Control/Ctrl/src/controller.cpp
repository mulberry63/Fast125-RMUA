...
)
{
	...
	*/
	...
	u.orientation = desired_attitude;
...
void Controller::publish_ctrl(const Controller_Output_t& u, const ros::Time& stamp)
{
	...
	msg.thrust = u.thrust;
	msg.orientation.w = u.orientation.w();
	msg.orientation.x = u.orientation.x();
	msg.orientation.y = u.orientation.y();
	msg.orientation.z = u.orientation.z();
	/*hzc*/
	...
	// 				0,-1,0,
	// 				0,0,-1;
	// Eigen::Quaterniond fianq(enutoned*u.orientation.toRotationMatrix()*body1tobody2);
	// Eigen::Vector3d eulerAngles;
	// eulerAngles = quaternion_to_ypr(fianq);
	// // eulerAngles = fianq.matrix().eulerAngles(1,0,2);//210 201x 
	// // pitch, roll, yaw 
	// double roll,pitch,yaw;
	// yaw = eulerAngles[0];
	// pitch = eulerAngles[1];
	// roll = eulerAngles[2];
	// msg.body_rate.x = roll;
	// msg.body_rate.y = pitch;
	// msg.body_rate.z = yaw;
	// Eigen::Quaterniond iterq(enutoned*u.orientation.toRotationMatrix());
	// std::cout << "iterq orientation, x: " <<iterq.x()<<" y: "<<iterq.y()
	// <<" z: "<<iterq.z()<<" w: "<<iterq.w()<<std::endl;
	// std::cout << "final orientation, x: " <<fianq.x()<<" y: "<<fianq.y()
	// <<" z: "<<fianq.z()<<" w: "<<fianq.w()<<std::endl;
