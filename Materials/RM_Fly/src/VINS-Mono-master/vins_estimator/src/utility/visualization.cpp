...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header)
{
    ...
    odometry.pose.pose.position.x = (tmp_R * P).x();
    odometry.pose.pose.position.y = (tmp_R * P).y();
    odometry.pose.pose.position.z = (tmp_R * P).z();

    Eigen::Quaterniond tmp_q(tmp_R * quadrotor_Q * tmp_R1);
    odometry.pose.pose.orientation.x = (tmp_q).x();
    odometry.pose.pose.orientation.y = (tmp_q).y();
    odometry.pose.pose.orientation.z = (tmp_q).z();
    odometry.pose.pose.orientation.w = (tmp_q).w();
    odometry.twist.twist.linear.x = (tmp_R * V).x();
    odometry.twist.twist.linear.y = (tmp_R * V).y();
    odometry.twist.twist.linear.z = (tmp_R * V).z();

    // wxx
    static bool pub_odom_flag = false;
    Eigen::Vector3d odom_p, odom_v;
    odom_p(0) = odometry.pose.pose.position.x;
    odom_p(1) = odometry.pose.pose.position.y;
    odom_p(2) = odometry.pose.pose.position.z;
    odom_v(0) = odometry.twist.twist.linear.x;
    odom_v(1) = odometry.twist.twist.linear.y;
    odom_v(2) = odometry.twist.twist.linear.z;

    if (!pub_odom_flag)
    {
        if( odom_p.norm() < 10.0 && odom_p.norm() > 1.5 && odom_v.norm() < 14.0 )
        {
