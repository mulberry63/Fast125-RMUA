...
public:
    ...
    {
        _odom_sub =
            handle.subscribe("odometry", 1, &TrajectoryServer::rcvOdometryCallback, this, ros::TransportHints().tcpNoDelay());

        _traj_sub =
            handle.subscribe("trajectory", 2, &TrajectoryServer::rcvTrajectoryCallabck, this, ros::TransportHints().tcpNoDelay());

        _predict_sub  =
            handle.subscribe( "front_pos_forpredict", 1, &TrajectoryServer::frontPosPredictCallback, this, ros::TransportHints().tcpNoDelay());
        _yaw_sub =
            handle.subscribe("desired_yaw", 1, &TrajectoryServer::rcvDesiredYaw, this, ros::TransportHints().tcpNoDelay());
        _cmd_pub =
    ...
    void rcvOdometryCallback(const nav_msgs::Odometry & odom)
    {
        ...
        double dt = _odom.header.stamp.toSec() - last_time;

        last_time = _odom.header.stamp.toSec();
        ...
        if (state == TRAJ)
        {
            ...
            //mpc ref path publish~
            {
                ...
                for(int k = 0; k < Ksample + 1; k++){
                    ...
                    mpc_ref_point.heading = _desired_yaw;
                    mpc_ref_point.heading_rate = 0.0;
                    mpc_ref_point.heading_acceleration = 0.0;
