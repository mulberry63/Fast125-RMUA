...
{
    waypoints_sub_ = nh_.subscribe("/goal", 1, &PlanManager::rcvGoalCallback, this, ros::TransportHints().tcpNoDelay());
    circle_sub_ = nh_.subscribe("circle_odom", 1, &PlanManager::rcvCircleCallback, this, ros::TransportHints().tcpNoDelay());
    odom_sub_ = nh_.subscribe("odom", 1, &PlanManager::rcvOdometryCallback, this, ros::TransportHints().tcpNoDelay());
    pg_to_vio_sub_ = nh_.subscribe("pg_T_vio", 1, &PlanManager::rcvPgToVioCallback, this, ros::TransportHints().tcpNoDelay());
    ...
    std::vector<double> tmp_dakeai_gate_pos_vec;
    nh_.getParam("dakeai_gate_pos_vec", tmp_dakeai_gate_pos_vec);

    for(int i = 0; i < tmp_dakeai_gate_pos_vec.size() / 3; i++) {
        Eigen::Vector3d tmp_position;
        tmp_position << tmp_dakeai_gate_pos_vec[3 * i + 1], tmp_dakeai_gate_pos_vec[3 * i], tmp_dakeai_gate_pos_vec[3 * i + 2];
        dakeai_gate_pos_list_.push_back(tmp_position);
    }
}
...
void PlanManager::ReplanCallback(const ros::TimerEvent& event)
{
    ...
    std::vector<Eigen::Vector3d>& gate_pos_list = get_dakeai_glb_traj_ ? dakeai_gate_pos_list_ : gate_pos_list_;
    ...
    if (((ros::Time::now() - lc_start_timestamp_).toSec() + replan_duration_) > duration)
    {
        ...
        gate_pos_list[replan_gate_index] = gate_pos_list[replan_gate_index] + pg_to_vio_;
    }
    else
    {
    ...
    if (replan_gate_index == gate_pos_list.size())
    {
        return;
    }

    if (get_dakeai_glb_traj_ && replan_gate_index == 2)
    {
        // wxx
        double tmp_max_vel;
        nh_.getParam("dakeai_dakeai_max_vel", tmp_max_vel);
        lcTrajOptPtr_ -> setVel(tmp_max_vel);
        double temp_rhoT;
        nh_.getParam("dakeai_dakeai_rhoT", temp_rhoT);
        lcTrajOptPtr_ -> setRhoT(temp_rhoT);

        global_se3_replan(replan_gate_index, gate_pos_list);
    }
    else
    {
        global_replan(replan_gate_index, gate_pos_list);
    }
}
...
void PlanManager::rcvOdometryCallback(const nav_msgs::Odometry & odom)
{
    ...
    if (!get_glb_traj_ && odom_p_.norm() < 10.0 && odom_p_.norm() > 1.5 && odom_v_.norm() < 14.0)
    {
        ...
        Eigen::Vector3d target_pos(tmp_target_p[1], tmp_target_p[0], tmp_target_p[2]);

        global_plan(gate_pos_list_, target_pos);
    }

    if (!get_dakeai_glb_traj_ && circle_12_->Istruth == true)
    {
        for (int i = 0; i < dakeai_gate_pos_list_.size(); i++)
        {
            dakeai_gate_pos_list_[i] += pg_to_vio_;
        }
        last_pg_to_vio_ = pg_to_vio_;

        std::vector<double> tmp_target_p;
        nh_.getParam("dakeai_target_pos", tmp_target_p);

        Eigen::Vector3d target_pos(tmp_target_p[1], tmp_target_p[0], tmp_target_p[2]);
        target_pos += pg_to_vio_;

        double tmp_max_vel;
        nh_.getParam("dakeai_max_vel", tmp_max_vel);
        lcTrajOptPtr_ -> setVel(tmp_max_vel);

        // wxx
        double temp_rhoT;
        nh_.getParam("dakeai_dw_rhoT", temp_rhoT);
        lcTrajOptPtr_ -> setRhoT(temp_rhoT);

        global_plan(dakeai_gate_pos_list_, target_pos);

        gate_index_ = -1;
        get_dakeai_glb_traj_ = true;
        get_lc_traj_ = false;
    }
}
...
void PlanManager::rcvCircleCallback(const geometry_msgs::PoseStamped& msg)
{
    ...
    for (int i = 0; i < gate_pos_list.size(); i++)
    {
        dist_temp = (gate_pos_list[i] - circle_pos).norm();
    ...
    {
        if (nearest_gate_index == gate_pos_list.size() - 1 && get_dakeai_glb_traj_)
        {
            double x, y, z;
        ...
        gate_pos_list[nearest_gate_index] = circle_pos * alpha_gate_update_ + gate_pos_list[nearest_gate_index] * (1 - alpha_gate_update_);
    }
    ...
    else
    {
        ...
        else
        {
            if (nearest_gate_index == gate_pos_list.size() - 1  && get_dakeai_glb_traj_)
            {
                ...
                global_se3_replan(nearest_gate_index, gate_pos_list);
            }
            else
            {
                if (get_lc_traj_)
                ...
                global_replan(nearest_gate_index, gate_pos_list);
            }
        }
    }
}
...
void PlanManager::global_plan(const std::vector<Eigen::Vector3d>& gate_pos_list, const Eigen::Vector3d& target_pos)
{
    start_p_ = odom_p_;
    start_v_ << odom_v_;
    start_a_ << 0, 0, 0;

    target_p_ << target_pos;
    target_v_ << 0, 0, 0;
    target_a_ << 0, 0, 0;
    ...
    for (int i = 0; i < N - 1; i++)
    {
      waypoints.col(i + 1) = gate_pos_list[i];
    }
...
void PlanManager::global_replan(const int& gate_index, const std::vector<Eigen::Vector3d>& gate_pos_list)
{
    if(!get_glb_traj_)
    {
        return;
    ...
    nh_.getParam("N_1", N_1);
    nh_.getParam("N_2", N_2);
    int N = N_1 + N_2 * (gate_pos_list.size() - gate_index);
    ...
    double tmp_t = replan_t;
    Eigen::VectorXd dt(gate_pos_list.size() - gate_index + 1);


    dt[0] = get_lc_traj_
          ? (cruise_traj.getDurations().sum() - cruise_traj.getDurations().tail((gate_pos_list.size() - gate_index) * N_2).sum() - replan_t) / N_1
          : (cruise_traj.getDurations().sum() - cruise_traj.getDurations().tail(gate_pos_list.size() - gate_index).sum() - replan_t) / N_1;
    for (int i = 1; i < gate_pos_list.size() - gate_index + 1; i++)
    {
        dt[i] = get_lc_traj_
              ? cruise_traj.getDurations().tail((gate_pos_list.size() - gate_index - i + 1) * N_2).head(N_2).sum() / N_2
              : cruise_traj.getDurations().tail(gate_pos_list.size() - gate_index - i + 1).head(1).sum() / N_2;
    }
    ...
    if (get_dakeai_glb_traj_)
    {
        res = lcTrajOptPtr_->generate_traj_r2(initState, finState, waypoints.block(0, 1, 3, N-1), Ts, fixed, gate_pos_list[gate_index], N_1 - 1, lc_traj_);
    }
    else
    {
        res = lcTrajOptPtr_->generate_traj_r2_dakeai(initState, finState, waypoints.block(0, 1, 3, N-1), Ts, fixed, gate_pos_list[gate_index], N_1 - 1, lc_traj_);
    }
...
void PlanManager::global_se3_replan(const int& gate_index, const std::vector<Eigen::Vector3d>& gate_pos_list)
{
    ...
    if (!get_se3_traj_)
    {
        finState << gate_pos_list[gate_index],
...
void PlanManager::visualize_rviz_moving_circle()
{
    ...
    double radius = 0.9;
    Eigen::Vector3d circle_12_position = circle_12_->getPos(odom_timestamp_);
    Eigen::Vector3d circle_13_position = circle_13_->getPos(odom_timestamp_);
    Eigen::Vector3d circle_12_vel = circle_12_->getVel(odom_timestamp_);
    Eigen::Vector3d circle_13_vel = circle_13_->getVel(odom_timestamp_);
...
{
    Trajectory cruise_traj = get_lc_traj_ ? lc_traj_ : glb_traj_;
    ros::Time cruise_timestamp = get_lc_traj_ ? lc_start_timestamp_ : glb_start_timestamp_;
    double t = (ros::Time::now() - cruise_timestamp).toSec();

    Eigen::Vector3d vel = cruise_traj.getVel(t);

    // wxx
    if( !get_dakeai_glb_traj_ && gate_index_ == 8 )
        return -M_PI / 15;
