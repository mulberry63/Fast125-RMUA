...
private:
    ...
    // for dakeai rm
    bool get_dakeai_glb_traj_ = false;
    std::vector<Eigen::Vector3d> dakeai_gate_pos_list_;
...
public:
    ...
    void global_plan(const std::vector<Eigen::Vector3d>& gate_pos_list, const Eigen::Vector3d& target_pos);

    void local_replan(const int& gate_index);

    void global_replan(const int& gate_index, const std::vector<Eigen::Vector3d>& gate_pos_list);

    void global_se3_replan(const int& gate_index, const std::vector<Eigen::Vector3d>& gate_pos_list);
