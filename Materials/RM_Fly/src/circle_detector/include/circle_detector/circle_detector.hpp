...
public:
    ...
    Circle_Detector (const ros::NodeHandle &nh):nh_(nh)
    {
        ...
        debug_slerp_pub = nh_.advertise<nav_msgs::Odometry>("/debug_slerp",1); 
    ...
    {   
        ...
        if (have_odom_msg)
        {
            ...
            double depth_odom_time_gap = (depth_img_msg->header.stamp - odom_vec.back().header.stamp).toSec();
            std::cout << std::setprecision(19) << "depth odom time gap = " << depth_odom_time_gap << std::endl;
            // geometry_msgs::PoseStamped debug_msg;
            // debug_msg.pose.position.x = depth_odom_time_gap;
            ...
            get_sync_odom(odom_vec,depth_img_msg,syn_odom_D);
            ...
            if (G_have_result)
            {
                ...
                circle_pos_msg.pose.orientation.w = G_result_odom.norm();
            ...
            // if (T_have_result && (current_odom(0) > gate_list[6](0)))
            if (T_have_result)
            {
                geometry_msgs::PoseStamped circle_pos_msg;
                ...
                circle_pos_msg.pose.orientation.w = T_result_odom.norm();
                circle_pos_pub.publish(circle_pos_msg);
            ...
            if (R_have_result)
            {
                ...
                if(flag_sperate)
                {
                    ...
                    circle_pos_msg.pose.orientation.w = R_result_odom.norm();
                    circle_pos_pub.publish(circle_pos_msg);
            ...
            if(A_have_result)
            {
                ...
                circle_pos_msg.pose.orientation.w = A_result_odom.norm();
                circle_pos_pub.publish(circle_pos_msg);
    ...
    bool slerp( const nav_msgs::Odometry &odom_front, const nav_msgs::Odometry &odom_back, 
                nav_msgs::Odometry & result_odom,ros::Time target_t)
    {
        Eigen::Quaterniond q_front(odom_front.pose.pose.orientation.w,
                                    odom_front.pose.pose.orientation.x,
                                    odom_front.pose.pose.orientation.y,
                                    odom_front.pose.pose.orientation.z);

        Eigen::Quaterniond q_back(odom_back.pose.pose.orientation.w,
                                    odom_back.pose.pose.orientation.x,
                                    odom_back.pose.pose.orientation.y,
                                    odom_back.pose.pose.orientation.z);
        Eigen::Vector3d p_front(odom_front.pose.pose.position.x,
                                odom_front.pose.pose.position.y,
                                odom_front.pose.pose.position.z);

        Eigen::Vector3d p_back(odom_front.pose.pose.position.x,
                                odom_front.pose.pose.position.y,
                                odom_front.pose.pose.position.z);

        double t = ((target_t - odom_front.header.stamp).toSec() - config.frame_delay) / (odom_back.header.stamp - odom_front.header.stamp).toSec();
        if( t > 1.0 || t < 0.0)
        {
            return false;
        }
        Eigen::Quaterniond lerp_q = q_front.slerp(t,q_back);
        lerp_q.normalize();
        Eigen::Vector3d tmp_p;
        tmp_p = (1 -t) * p_front + t * p_back;
        result_odom.pose.pose.position.x = tmp_p(0);
        result_odom.pose.pose.position.y = tmp_p(1);
        result_odom.pose.pose.position.z = tmp_p(2);
        result_odom.pose.pose.orientation.w = lerp_q.w();
        result_odom.pose.pose.orientation.x = lerp_q.x();
        result_odom.pose.pose.orientation.y = lerp_q.y();
        result_odom.pose.pose.orientation.z = lerp_q.z();

        nav_msgs::Odometry slerp_msg;
        slerp_msg = result_odom;
        slerp_msg.header.stamp = odom_back.header.stamp;
        slerp_msg.header.frame_id = "world";
        debug_slerp_pub.publish(slerp_msg);

        // static int count = 0; 
        // std::cout << "slerp   t = " << t << std::endl;
        // std::cout << "[circle odom debug] slerp count : " << count << std::endl;
        // count++;

        return true;
    }

    void get_sync_odom(const std::vector<nav_msgs::Odometry> & odom_vec,
                             const sensor_msgs::ImageConstPtr & depth_img_msg,
                             nav_msgs::Odometry &syn_odom_D )
    {
        int index = -1;
        double min_time_gap = 1e10;
        double signed_time_gap = 0.0;
        for (int k = odom_vec.size() ; k > 0 ;k--)
        {
            double tmp_sign_time_gap = (depth_img_msg->header.stamp - odom_vec[k-1].header.stamp).toSec() - config.frame_delay;
            double temp_t = std::abs(tmp_sign_time_gap);
            if (temp_t < min_time_gap)  
            {
                signed_time_gap = tmp_sign_time_gap;
                min_time_gap = temp_t;
                syn_odom_D = odom_vec[k-1];
                index = k-1;
            }
        ...
        if (signed_time_gap == 0) return;

        if (signed_time_gap > 0 && index < odom_vec.size() -1)
        {
            nav_msgs::Odometry tmp_odom_front = odom_vec[index];
            nav_msgs::Odometry tmp_odom_back = odom_vec[index + 1];
            nav_msgs::Odometry tmp_result;
            bool sync_flag = slerp(tmp_odom_front , tmp_odom_back,tmp_result , depth_img_msg->header.stamp);  
            if (!sync_flag) return ;
            syn_odom_D = tmp_result;
            syn_odom_D.header.stamp.fromSec(depth_img_msg->header.stamp.toSec() - config.frame_delay);
            syn_odom_D.header.frame_id = tmp_odom_front.header.frame_id;
        }
        else if(signed_time_gap < 0 && index > 0)
        {
            nav_msgs::Odometry tmp_odom_front = odom_vec[index -1];
            nav_msgs::Odometry tmp_odom_back = odom_vec[index];
            nav_msgs::Odometry tmp_result;
            bool sync_flag = slerp(tmp_odom_front , tmp_odom_back,tmp_result , depth_img_msg->header.stamp);  
            if (!sync_flag) return ;
            syn_odom_D = tmp_result;
    ...
    */
    ...
    bool dealWithTransition(std::vector<std::pair<Eigen::Vector3d,ros::Time>> &T_pos_stamp_pair_vec,double &vel,double &left_endpoint_y)
    {
        ...
        if( vel_vec.size() == 0  )
        {
            for (int i = 0; i < T_pos_stamp_pair_vec.size() - 1; i++)
            {
        ...
        int current_vel_direction = 0;
        int count = 0;
        int direction_num = 5;
        for (int i = T_pos_stamp_pair_vec.size() - 1, k = 0; k < direction_num ;k++)
        {
            double temp_dis = T_pos_stamp_pair_vec[i - k].first(1) - T_pos_stamp_pair_vec[i - k -1].first(1);

            count = temp_dis > 0 ?  count + 1 : count - 1;           
        }

        bool vel_direction_truth = false;
        if (count >= direction_num -2 || count <= -(direction_num - 2))
        {
            vel_direction_truth = true;
            current_vel_direction = count >= (direction_num -2) ? 1 : -1;
        }
        else current_vel_direction = last_vel_direction;

        geometry_msgs::PoseStamped debug_msg;
        debug_msg.pose.position.x = vel;
        
        
        // debug_msg.pose.orientation.w = (T_pos_stamp_pair_vec.back().first(1) - T_pos_stamp_pair_vec.begin()->first(1)) /
        //                                 (T_pos_stamp_pair_vec.back().second - T_pos_stamp_pair_vec.begin()->second).toSec(); 

        bool flag = false;

        bool vel_variance_accept = false;
        double variance = 0.0;
        if (vel_vec.size() > config.max_continue_count)
        {
            // double mean_vel = std::accumulate(vel_vec.begin(),vel_vec.end(),0.0);
            double mean_vel = 0.0;
            ...
            mean_vel = mean_vel / vel_vec.size();

            for (int i = 0; i < vel_vec.size(); i++)
            {   
                variance += (vel_vec[i] - mean_vel) * (vel_vec[i] - mean_vel); 
            }
            variance = std::sqrt(variance / vel_vec.size());
            vel_vec.erase(vel_vec.begin());  
            if (variance < config.variance_tolerance) 
            {
                vel_variance_accept = true;
            }      
        } 
        debug_msg.pose.orientation.x = vel_variance_accept;
        debug_msg.pose.orientation.y = variance;
        ...
        double award_thres = 0;
        if ( vel_direction_truth && vel_variance_accept)
        {
            if( vel < 2.1 )
                award_thres = 1.2;
            else if( vel > 2.2 )
        ...
        if ((curr_pos_y - transition_gate(1) - award_thres) < 0 && current_vel_direction > 0)
        {
            flag = true;
        }
        if ((curr_pos_y - transition_gate(1) + award_thres) > 0 && current_vel_direction < 0)
        ...
        debug_msg.pose.orientation.z = flag;
        debug_msg.pose.position.z = current_vel_direction;
        debug_msg.pose.position.y = curr_pos_y - transition_gate(1);

        left_endpoint_y = current_vel_direction > 0 ? min_y + 6.54 : max_y;
        flag = flag && vel_direction_truth && vel_variance_accept;
        debug_msg.pose.orientation.w = flag;
...
# define DEBUG(true)
    ...
    bool Determine_circle_center_by_pointcloud(const std::vector<Eigen::Vector3d> &input_cloud,Eigen::Vector3d &circle_center)
    {
        ...
        std::vector<Eigen::Vector3d> sort_points;
        std::vector<Eigen::Vector3d> sort_points_raw;
        sort_points_raw.clear();
        for (int i = 0 ; i < POINTNUMS ; ++i)
        {
            Eigen::Matrix<double,3,1> displacement = tempPoints[i] -  circle_center;
            Eigen::Matrix<double,3,1> displacement_R = evector.transpose() * displacement;
            sort_points.push_back(displacement_R);
            sort_points_raw.push_back(displacement);

            if (displacement_R(2) < minExtents(2))
            {
                have_left = true;
                minExtents(2) = displacement_R(2);
                left_endpoint = displacement;
            }
            else if (displacement_R(2) > maxExtents(2))
            {
                have_right = true;
                maxExtents(2) = displacement_R(2);
                right_endpoint = displacement;
            }
        ...
        std::sort(sort_points.begin(),sort_points.end(),[](Eigen::Vector3d a, Eigen::Vector3d b){return (a(2) < b(2)) ;});
        int center_num = 10;
        std::vector<Eigen::Vector3d> middle_point_vec;
        middle_point_vec.clear();
        // int sort_points_index_increment = sort_points.size() / ( center_num + 1 );
        // int index = sort_points_index_increment;
        // for( int i = 0; i < center_num; i++ )
        // {
        //     Eigen::Vector3d temp_pont = sort_points[index];
        //     Eigen::Vector3d temp_norm = (evector.transpose()).inverse() * temp_pont;
        //     std::vector<Eigen::Vector3d> temp_sort_points = sort_points_raw;
        //     std::sort(temp_sort_points.begin(),temp_sort_points.end(),
        //               [&](Eigen::Vector3d a, Eigen::Vector3d b){return (a.dot(temp_norm) > b.dot(temp_norm)) ;});
        //     middle_point_vec.push_back(temp_sort_points[0]);

        //     index += sort_points_index_increment;    
        // }
        middle_point_vec.push_back(middle_point);
        // middle_point_vec.push_back(middle_point);
        // middle_point_vec.push_back(middle_point);
        // // middle_point_vec.push_back(sort_points_raw[100]);

        if(have_left && have_right && have_middle)
        {
            Eigen::Vector3d mean_circle_center(0,0,0);
            for(int i = 0;i < middle_point_vec.size() ; i++)
            {
                Eigen::Vector3d local_temp_middle = middle_point_vec[i];
                Eigen::Vector3d tmp_left = evector.transpose() * left_endpoint ;
                Eigen::Vector3d tmp_right = evector.transpose() * right_endpoint ;
                Eigen::Vector3d tmp_middle = evector.transpose() * local_temp_middle ;
                ...
                Eigen::Vector3d temp_circle_center = (evector.transpose()).inverse() * Eigen::Vector3d(0,intersection(0),intersection(1)) + circle_center;

                std::cout << "[circle odom debug t_] :   " << t_ << std::endl;
                std::cout << "[dedbug circle center]    :   " << circle_center.transpose() << std::endl;

                vis_point_vec.push_back(temp_circle_center);
                vis_point_vec.push_back(left_endpoint);
                vis_point_vec.push_back(right_endpoint);
                vis_point_vec.push_back(local_temp_middle);

                mean_circle_center += temp_circle_center;
                // std::cout <<"[circle odom debug] circle_center : " << circle_center.transpose() << std::endl;
                // std::cout <<"[circle odom debug] left_endpoint : " << left_endpoint.transpose() << std::endl;
                // std::cout <<"[circle odom debug] right_endpoint : " << right_endpoint.transpose() << std::endl;
                // std::cout <<"[circle odom debug] middle_point : " << middle_point.transpose() << std::endl;
            ...
            mean_circle_center = mean_circle_center / middle_point_vec.size();
            circle_center = mean_circle_center;
            vis_point_vec.push_back(mean_circle_center);
            visualizer.vis_pointcloud(vis_point_vec,partial_detect_result_vis);
